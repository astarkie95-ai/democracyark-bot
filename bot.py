...
def _render_starter_admin_box() -> str:
    ...
    claimed = len(CLAIMS)
    storage = "Database" if DB_ENABLED else "CSV files"
    return _module_box(
        "STARTER VAULT ADMIN",
        [
...
class _StarterAdminAddModal(discord.ui.Modal, title="Add Starter Vault(s)"):
    pins_text = discord.ui.TextInput(
        label="Vaults to add",
        style=discord.TextStyle.paragraph,
        required=True,
        placeholder="Examples:\n1,1234\n2:5678\n9999 (auto-assign next vault #)",
        max_length=2000,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for line in raw.splitlines():
            ...
            if "," in line:
                ...
                # Could be a vault # or a pin; treat short numbers as vault # only if format is explicit
                pin = line

            if pin is None or not str(pin).strip().isdigit():
                skipped.append(f"{line} (pin must be digits)")
                continue
            pin = str(pin).strip()
            ...
            if box in PINS_POOL or box in CLAIMS:
                skipped.append(f"{line} (vault #{box} already used)")
                continue
            if pin in used_pins:
                skipped.append(f"{line} (pin already used)")
                continue

            PINS_POOL[box] = BoxPin(box=box, pin=pin)
            used_pins.add(pin)
            added.append(f"Vault #{box}")
        ...
        if skipped:
            ...
            if len(skipped) > 20:
                msg.append(f"...and {len(skipped) - 20} more.")
        if not msg:
            msg.append("❌ Nothing was added.")
        await interaction.response.send_message("\n\n".join(msg), ephemeral=True)

class _StarterAdminDeleteModal(discord.ui.Modal, title="Delete Starter Vault(s)"):
    targets_text = discord.ui.TextInput(
        label="Vault # or PIN to delete",
        style=discord.TextStyle.paragraph,
        required=True,
        placeholder="Examples:\n1\n2\n1234 (PIN)",
        max_length=2000,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        raw = str(self.targets_text.value or "").strip()
        if not raw:
        ...
        for line in raw.splitlines():
            ...
            # Prefer vault number if it matches an existing vault key
            if t.isdigit() and int(t) in PINS_POOL:
                box = int(t)
                PINS_POOL.pop(box, None)
                removed.append(f"Vault #{box}")
                continue
        ...
        msg = []
        if removed:
            msg.append("✅ Deleted: " + ", ".join(removed))
        if skipped:
            msg.append("⚠️ Skipped:\n" + "\n".join(skipped[:20]))
            if len(skipped) > 20:
                msg.append(f"...and {len(skipped) - 20} more.")
        if not msg:
            msg.append("❌ Nothing was deleted.")
        await interaction.response.send_message("\n\n".join(msg), ephemeral=True)
